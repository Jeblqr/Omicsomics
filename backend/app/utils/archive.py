...
def extract_zip_file(
    zip_path: Path, extract_to: Path, member_name: Optional[str] = None
) -> Path:
    """
...
def extract_tar_file(
    tar_path: Path, extract_to: Path, member_name: Optional[str] = None
) -> Path:
    """
                ...
                for member in tf.getmembers():
                    ...
                    if not is_safe_path(extract_to, target_path):
                        raise UnsafeArchiveError(
                            f"Path traversal attempt: {member.name}"
                        )
...
def extract_archive(
    archive_path: Path, extract_to: Path, member_name: Optional[str] = None
) -> Path:
    """
