...
class GenomicsConverter(BaseConverter):
    ...
    async def _vcf_to_unified(
            ...
            for line in f:
                if line.startswith("##"):
                    ...
                    if line.startswith("##fileformat="):
                        metadata.custom_fields["vcf_version"] = line.strip().split("=")[
                            1
                        ]
                    continue
        ...
        return GenomicsData(
            metadata=metadata, headers=headers, records=records, statistics=statistics
        )
    ...
    async def _unified_to_vcf(
        ...
        with open_func(output_path, mode) as f:
            ...
            f.write(
                f"##reference={unified_data.metadata.reference_genome or 'unknown'}\n"
            )
