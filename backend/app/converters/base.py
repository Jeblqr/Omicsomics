...
class BaseConverter(ABC):
    ...
    def add_processing_step(
        ...
        **parameters,
        ...
        step = ProcessingStep(
            step_name=step_name, tool=tool, version=version, parameters=parameters
        )
        unified_data.metadata.processing_steps.append(step)
        return unified_data

    async def save_unified(self, unified_data: UnifiedData, output_path: Path) -> Path:
        """Save unified data to JSON file."""
        output_path.parent.mkdir(parents=True, exist_ok=True)

        with open(output_path, "w") as f:
            json.dump(unified_data.model_dump(mode="json"), f, indent=2, default=str)
